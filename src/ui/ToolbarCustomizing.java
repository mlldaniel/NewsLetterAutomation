 ...
 */
public class ToolbarCustomizing {
    
    private JFrame frame = null;
    private ModifiedEditor editor;
    
    ChromePreviewThread chromePreviewThread;
    Thread t;
    
    /**
     * An {@link BaseEditor} with modified toolbar components
     */
    public static class ModifiedEditor extends BaseEditor {
...
//    }
    public void create(Component buttonForm) {
        if (editor == null) {
            editor = new ModifiedEditor();
            frame = new JFrame("Preview HTML EDITOR");
            frame.setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
            frame.setLayout(new BorderLayout());
            frame.add(editor, BorderLayout.CENTER);
            frame.add(buttonForm, BorderLayout.SOUTH);
            frame.setSize(597, 600);
    ...
    public void open(String html) {
        editor.setText(html, true);
            
            chromePreviewThread = new ChromePreviewThread(editor,frame);
            t = new Thread(chromePreviewThread);
            t.start();
        
        frame.setVisible(true);
    }

    public String saveNClose() {
        frame.setVisible(false);
        return editor.getText();
    }

    public void discardNclose() {
        frame.setVisible(false);
        return;
    }
